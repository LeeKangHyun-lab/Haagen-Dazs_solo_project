...
public class PageHelper {
    ...
    private int groupCount; // 한 화면에 표시될 페이지번호 개수

    /***** 연산결과 값 ******/
    private int totalPage;  // 전체 페이지 수
    private int startPage;  // 현재 페이지의 시작 페이지
    private int endPage;    // 현재 페이지의 마지막 페이지
    private int prevPage;   // 이전 페이지
    private int nextPage;   // 다음 페이지
    private int offset;     // MySQL, MariaDB의 Limit 시작 위치

    /**
     * 객체가 만들어질 동시에 필요한 연산을 수행하는 생성자.
     */
    public PageHelper(int nowPage, int totalCount, int listCount, int groupCount) {
        ...
        // 이전 그룹의 마지막 페이지
        prevPage = (startPage - groupCount) > 0 ? startPage - 1 : 0;
